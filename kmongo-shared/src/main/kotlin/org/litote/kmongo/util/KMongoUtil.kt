...
object KMongoUtil {
    ...
    fun <T : Any> toBson(json: String, type: Class<T>): BsonDocument =
            generateIfAbsentAndMayBeMoveId(toBson(json), type)

    private fun <T : Any> generateIfAbsentAndMayBeMoveId(document: BsonDocument, type: Class<T>): BsonDocument {
        if (!document.containsKey("_id")) {
            val idProperty = ClassMappingType.findIdProperty(type)
            if (idProperty != null) {
                val idValue = document.get(idProperty.name)
                if (idValue == null) {
                    val toString = idProperty.type.toString()
    ...
    fun extractId(obj: Any, clazz: Class<*>): Any {
        //check map
        if (obj is Map<*, *>) {
    ...
    private fun <T : Any> toWriteModel(bson: BsonDocument, type: Class<T>): WriteModel<BsonDocument> {
    ...
    fun getIdValue(value: Any): Any? {
        ...
        val idProperty = ClassMappingType.findIdProperty(value.javaClass)
        @Suppress("UNCHECKED_CAST")
